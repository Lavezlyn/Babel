...
# Add project root to path so that imports work.
...
def main():
    ...
    parser.add_argument("--backend", type=str, default="dummy",
                       choices=["openai", "vllm", "deepseek", "dummy"],
    ...
    if args.backend == "openai":
        ...
        # DeepSeek backend expects DEEPSEEK_API_KEY env var or --api-key
        if not args.api_key:
            args.api_key = os.environ.get("DEEPSEEK_API_KEY")
        if not args.api_key:
            raise ValueError("--api-key or DEEPSEEK_API_KEY environment variable required for DeepSeek backend")
        llm_config.update({
            "model": args.model,
            "api_key": args.api_key,
            "api_base": args.api_base,
            "cache_dir": f"{args.cache_dir}/deepseek",
            "use_cache": not args.no_cache,
        })
